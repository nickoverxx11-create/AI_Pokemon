...
namespace toio.Simulator
{
    ...
    public class StandardID : MonoBehaviour
    {
        ...
        public static string GetCardNameByID(uint standardId)
        {
            string idString = standardId.ToString();
            ...
            uint cardID = uint.Parse(lastTwoDigits);

            int index = Array.IndexOf(SimpleCardIDs, cardID);
            if (index >= 0 && index < SimpleCardNames.Length)
            {
                return SimpleCardNames[index];
            }
